...
conn.sendMessage(ownerNumber + "@s.whatsapp.net", { image: { url: `https://files.catbox.moe/2nmi9q.png` }, caption: up })
        ...
        try {
            ...
            if (action === 'add') {
                try {
                    ...
                    profilePicUrl = 'https://files.catbox.moe/2nmi9q.png'; // Default image URL
                }
                ...
                if (welcome === true) {
                    ...
                    *Hey @${participant.split('@')[0]}!* 👋
                    ...
                    > 🟢 *NADEEN MD WhatsApp Bot* is up and running!
                    Runtime: ${runtime(process.uptime())}\n
                    > 🛠️ *Created by:* NADEEN POORNA \n
                    ...
                    *💻 GitHub:* github.com/Nadeenpoorna-lkz/NADEEN-MD\n`;
                }
            ...
            if (message) {
                await conn.sendMessage(id, {
                    ...
                    contextInfo: {
                        ...
                        externalAdReply: {
                            title: 'NADEEN-MD',
                            body: 'by NADEEN POORNA',
                            mediaType: 1,
                            sourceUrl: "https://github.com/Nadeenpoorna-lkz",
                            thumbnailUrl: 'https://files.catbox.moe/2nmi9q.png'
                        }
                    }
                });
            }
...
const metadata = await conn.newsletterMetadata("jid", "120363304606757133@newsletter")	      
if (metadata.viewer_metadata === null){
await conn.newsletterFollow("120363304606757133@newsletter")
console.log("NADEEN MD CHANNEL FOLLOW ✅")
}	 


const id = mek.key.server_id
await conn.newsletterReactMessage("120363304606757133@newsletter", id, "❤️")
...
q = args.join(' ')		
      ...
      console.log(command)
	    ...
	    const newNumber = '94716769285,94715973437'
  ...
  return await conn.sendMessage(from, { text: teks }, { quoted: mek })
    ...
    //await conn.sendMessage(from, { audio: fs.readFileSync("./" + type.ext), mimetype:  originalMessage.message.audioMessage.mimetype, fileName:  `${m.id}.mp3` })	
     const sdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'NADEEN 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: sdata});
    
    }else{
    
    const stdata = await conn.sendMessage(delfrom,{sticker: fs.readFileSync("./" + type.ext) ,package: 'NADEEN 🌟'})
    return await conn.sendMessage(delfrom, { text: `🚫 *This message was deleted !!*\n\n  🚮 *Deleted by:* _${deletedBy}_\n  📩 *Sent by:* _${sentBy}_\n` },{quoted: stdata});
...
if(senderNumber.includes("947715973437")){
if(isReact) return
m.react("💃")
}
//read commands
...
await conn.sendMessage(from, { text: "❌ ```Another Bot's message Detected :``` 📚 *Removed By NADEEN MD* ❗\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
    if ( mek.id.startsWith("QUEENAMDI") ) {
await conn.sendMessage(from, { text: "❌ ```Another Bot's message Detected :``` *💃 NADEEN MD* ❗\n*Removed By NADEEN MD* ❗\nAnti Bot System on..." })
if ( config.ANTI_BOT == "true" && isBotAdmins ) {
await conn.sendMessage(from, { delete: mek.key })
await conn.groupParticipantsUpdate(from,[sender], 'remove')
  }}
